...
if __name__ == "__main__":
    ...
    doc = Document(content="""Moonlight shimmered softly, wolves howled nearby, night enveloped everything.
Leaves rustled in the breeze, shadows danced across the forest floor.
An owl blinked silently from its perch, ever watchful, ever still.
Far below, a stream murmured secrets to the stones lining its bed.
Somewhere in the distance, a branch cracked — quick, sharp, then silence.
The scent of pine and damp earth mingled, grounding the stillness.
A fox emerged briefly, then vanished into the thicket.
Stars glittered overhead, ancient and unmoved, bearing silent witness.
Time slowed, wrapped in hush, broken only by nature's quiet breath.
All things waited, listening.""")

    splitter = DocumentSplitter(split_by="word", split_length=20, split_overlap=3,respect_sentence_boundary=True)
